  ...
  Vue.auth = {
    ...
    login(email, password) {
      ...
      let promise = new Promise((resolve, reject) => {
        axios.post('http://api.gifkawood.ru/api/login', params)
          .then(response => {
    ...
    async check() {
      ...
      if (access_token) {
        axios.defaults.baseURL = "http://api.gifkawood.ru/api/checkaccesstoken";
        return true;
      } else {
        return false;
      }
    ...
    setToken(access_token) {
      ...
      axios.defaults.baseURL = 'http://api.gifkawood.ru/api/';
